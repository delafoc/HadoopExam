...
class map2 extends Mapper<LongWritable, Text, Text, LongWritable> {
    @Override
    protected void map(LongWritable key, Text value, Mapper<LongWritable, Text, Text, LongWritable>.Context context) throws IOException, InterruptedException {
        ...
        if (strings != null && strings.length >= 5 && strings[0].contains("山西省") && strings[4].contains("2013") && strings[1] != null) {
            if (Integer.parseInt(strings[1]) == 4)
                context.write(new Text("山西省" + strings[2] + strings[3]), new LongWritable(1));
        }
    }
}

class reduce2 extends Reducer<Text, LongWritable, Text, DoubleWritable> {
    ...
    protected void cleanup(Reducer<Text, LongWritable, Text, DoubleWritable>.Context context) throws IOException, InterruptedException {
        ...
        for (String s : keySet) {
            ...
            context.write(new Text("2013年山西省4月份"+ s + "地区销售数量为：" + value + "  总数量为：" + num + "\t" + "销售数量的比例为："), new DoubleWritable(percent));
        }
    }
}

public class hive1_3 {
...
//        String input = "/hivetest/test/11 汽车销售数据-Cars.csv";
        ...
        String output = "/hivetest/output/test1.3";

        //设置job名称
        job.setJobName("hive1.3");

        //set创建Jar包位置
        job.setJar("E:\\程序\\Java\\hadoopexam\\target\\hadoopexam-1.0-SNAPSHOT.jar");

        //主函数位置
        job.setJarByClass(hive1_3.class);
