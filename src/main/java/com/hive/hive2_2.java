...
class map4 extends Mapper<LongWritable, Text, Text, LongWritable> {
    @Override
    protected void map(LongWritable key, Text value, Mapper<LongWritable, Text, Text, LongWritable>.Context context) throws IOException, InterruptedException {
        byte[] bytes = value.getBytes();
        String str = new String(bytes, 0, value.getLength(), "GBK");
        String[] strings = str.trim().split(",");
        if (strings != null && strings.length > 9 && strings[5].trim().length() > 0 && strings[8].trim().length() > 0 && strings[9].trim().length() > 0)
            context.write(new Text(strings[9] + "\t" + strings[8] + "\t" + strings[5]), new LongWritable(1));
    }
}

class reduce4 extends Reducer<Text, LongWritable, Text, Text> {
    ...
    protected void setup(Reducer<Text, LongWritable, Text, Text>.Context context) throws IOException, InterruptedException {
        context.write(new Text("所有权" + "\t" + "类型" + "\t" + "型号" + "\t" + "数量"), new Text(""));
    }
...
//        String input = "/hivetest/test/11 汽车销售数据-Cars.csv";
        ...
        String output = "/hivetest/output/test2.2";

        //设置job名称
        job.setJobName("hive2.2");

        //set创建Jar包位置
        job.setJar("E:\\程序\\Java\\hadoopexam\\target\\hadoopexam-1.0-SNAPSHOT.jar");

        //主函数位置
        job.setJarByClass(hive2_2.class);
