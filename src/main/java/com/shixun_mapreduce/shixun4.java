...
class map4 extends Mapper<LongWritable, Text, Text, Text> {
    @Override
    protected void map(LongWritable key, Text value, Mapper<LongWritable, Text, Text, Text>.Context context) throws IOException, InterruptedException {
        String[] strings = value.toString().trim().split(",");
        if (strings[3].length() > 0 && strings[4].length() > 0)
            context.write(new Text(strings[3]), new Text(strings[4]));
    }
}

class reduce4 extends Reducer<Text, Text, Text, Text> {
...
//    HashMap<String, Long> off_whiteMap = new HashMap<String, Long>(); // off_white
    ...
    protected void setup(Reducer<Text, Text, Text, Text>.Context context) throws IOException, InterruptedException {
        context.write(new Text("运动鞋名称" + "\t" + "最低与最高发售价"), new Text(""));
    }
...
public class shixun4 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        String output = "/shixun/output4";

        //设置job名称
        job.setJobName("Job3_shixun4");

        //set创建Jar包位置
        job.setJar("E:\\程序\\Java\\hadoopexam\\target\\hadoopexam-1.0-SNAPSHOT.jar");

        //主函数位置
        job.setJarByClass(shixun4.class);
